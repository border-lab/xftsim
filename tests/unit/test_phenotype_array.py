...
class TestConstruction:
    ...
    def test_with_values(self):
        ...
        p = NPhenotypeArray(samples=sm, values=vals)
        assert 'A' in p
        assert 'B' in p
    ...
    def test_wrong_shape_raises(self):
        ...
        with pytest.raises(ValueError, match="shape"):
            NPhenotypeArray(samples=sm, values={'X': np.ones(5)})

    def test_values_coerced_to_float64(self):
        """Integer values should be coerced to float64."""
        sm = _make_samples(10)
        p = NPhenotypeArray(samples=sm, values={'X': np.ones(10, dtype=np.int32)})
...
class TestKeys:
    def test_keys_empty(self):
        ...
        p = NPhenotypeArray(samples=_make_samples())
        assert list(p.keys) == []
...
class TestSubset:
    def test_subset_data(self):
        ...
        sm = _make_samples(10)
        p = NPhenotypeArray(samples=sm, values={'X': np.arange(10, dtype=float)})
    ...
    def test_subset_preserves_all_keys(self):
        ...
        sm = _make_samples(10)
        p = NPhenotypeArray(samples=sm, values={
...
class TestRepr:
    def test_repr_contains_class(self):
        """Repr should mention NPhenotypeArray."""
        p = NPhenotypeArray(samples=_make_samples(5))
        assert 'NPhenotypeArray' in repr(p)
    ...
    def test_repr_shows_keys(self):
        ...
        sm = _make_samples(5)
        p = NPhenotypeArray(samples=sm, values={'alpha': np.ones(5)})
